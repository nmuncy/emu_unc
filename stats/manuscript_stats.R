...
ggplot(df_long, aes(x = mem, y = value, fill = group)) +
  ...
  annotate(
    "segment",
    x = 1, xend = 2, y = 0.6, yend = 0.6, color = "black"
  ) +
  annotate(
    "segment",
    x = 1, xend = 1, y = 0.6, yend = 0.57, color = "black"
  ) +
  annotate(
    "segment",
    x = 2, xend = 2, y = 0.6, yend = 0.57, color = "black"
  ) +
...
df_count <- as.data.frame(matrix(
  NA,
  nrow = length(subj_list) * length(beh_list), ncol = length(col_names)
))
...
ggplot(df_count, aes(x = valence, y = prop, fill = group)) +
  ...
  annotate(
    "segment",
    x = 1, xend = 2, y = 0.92, yend = 0.92, color = "black"
  ) +
  annotate(
    "segment",
    x = 1, xend = 1, y = 0.92, yend = 0.89, color = "black"
  ) +
  annotate(
    "segment",
    x = 2, xend = 2, y = 0.92, yend = 0.89, color = "black"
  ) +
